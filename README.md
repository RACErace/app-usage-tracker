...
一个仿照手机“使用统计”界面的桌面应用，用于统计 Windows 电脑上的应用使用时长；当目标应用是浏览器时，可通过附带的 Chrome / Edge 扩展识别到具体网页，并按根域名聚合显示。
...
- 支持每天视图与近 7 天视图
- 支持应用排行与详情页
- 浏览器场景下可识别具体网页标题、域名、URL，并按根域名合并统计
- 排行和详情页会优先显示应用真实图标与网站 favicon，失败时回退到首字母占位
- 已成功解析过的应用图标和网站 favicon 会缓存到本地图标缓存目录，减少重复请求
- 最小化和关闭窗口后常驻系统托盘
- 支持通过托盘菜单切换开机自启动
- 应用界面提供独立“设置”页，并以系统开关样式查看和切换开机自启动状态
- 主界面支持响应式布局，会根据窗口尺寸自动调整图表和列表排布
- 本地存储数据，不依赖云服务
...
3. 桌面端把两边数据合并，再按根域名归并，例如不同的 bilibili 视频页都会记到 bilibili 站点下。
...
`npm start` 会以脱离终端的方式启动桌面应用，因此关闭当前 PowerShell 窗口后，应用仍会继续运行。

如果你需要保留前台日志并让应用跟随终端生命周期运行，可使用：
...
- 右上角“设置”可进入独立设置页，查看并切换开机自启动
- 主界面会根据窗口大小自动调整为单列或双列布局
...
应用图标与网站 favicon 会缓存到 Electron userData 目录中的 icon-cache 目录。
