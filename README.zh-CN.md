...
- 统计 Windows 前台应用使用时长
- 对支持 SMTC 的音乐软件按实际播放状态计时，后台播放也会累计
...
3. 对支持 SMTC 的音乐软件，桌面端会额外读取系统媒体会话，把实际播放中的时长记到音乐应用
4. 桌面端合并这些事件，再按应用、站点或服务规则记账

如果不加载扩展，浏览器使用时长仍会被统计，但只能记到浏览器应用本身，无法细分到网站。

说明：

- 音乐播放时长来自 Windows SMTC（System Media Transport Controls）会话，暂停后不会继续累计
- 这类播放时长可能与其他前台应用同时发生，因此按项目汇总时，会和单纯“前台窗口时长”存在差异
- 不同音乐软件对 SMTC 的支持程度不同；未暴露媒体会话的播放器仍会退回前台窗口统计
...
- 音乐播放统计依赖应用对 Windows SMTC 的实现；浏览器中的网页播放器不会作为独立音乐应用额外记账
