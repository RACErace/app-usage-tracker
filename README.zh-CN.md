...
- 统计 Windows 前台应用使用时长
- 对音乐软件使用 SMTC + WASAPI 双源检测实际播放时间，后台播放也会累计
- 配合浏览器扩展后，可把浏览器使用归因到具体网站
- 网页按根域名聚合，而不是把每个 URL 单独当作一个项目
- 支持自定义“桌面应用 + 域名”服务合并规则
- 支持给应用、网站和服务配置“工作 / 娱乐 / 学习 / 沟通”等分类规则
- 提供日排行、近 7 天趋势和详情页
- 支持系统托盘、开机自启动和关闭行为设置
- 支持通过 CLI 查询本地统计数据
...
3. 对音乐软件，桌面端会同时读取 Windows SMTC 媒体会话和 WASAPI 音频会话
4. 桌面端融合这两路信号，把实际播放中的时长记到音乐应用
5. 再按应用、站点或服务规则统一记账
...
- 音乐播放时长优先使用 Windows SMTC（System Media Transport Controls）状态，并用 WASAPI 音频会话补足未暴露 SMTC 的播放器
- 这类播放时长可能与其他前台应用同时发生，因此按项目汇总时，会和单纯“前台窗口时长”存在差异
- 某些播放器如果既不暴露 SMTC，也没有活跃 WASAPI 会话，仍会退回前台窗口统计
...
- 服务合并和分类依赖你配置的规则；没有命中的项目会保持原始应用或站点形态
- 某些 favicon、图片或资源 URL 可能会被记录为最近访问网页
- 音乐播放统计仍依赖播放器暴露可识别的 SMTC 或 WASAPI 会话；浏览器中的网页播放器不会作为独立音乐应用额外记账
