...
App Usage Tracker 会记录 Windows 当前前台应用、浏览器活动站点，以及支持的音乐播放器是否真的在播放，再把这些原始信号整理成日排行、近 7 天趋势、详情页、本地备份和可自动化调用的 CLI。
...
| 根域名聚合 | 网页记录按根域名汇总，避免把每个 URL 都拆成独立项目 |
...
| 本地 CLI | 直接查询日期、排行、搜索、详情和完整 snapshot |
...
| 显示控制 | 可隐藏项目，使其不参与总时长、排行、搜索和 snapshot |
...
| 页面 | 做什么 |
|------|--------|
| 概览 | 查看每日 / 近 7 天图表、排行、当前总量和浏览器扩展状态 |
...
- CLI 会遵循 `settings.json` 中的显示设置，隐藏项目不会出现在总时长、排行、搜索和 snapshot 中
...
项目在 [`skills/app-usage-tracker-query`](./skills/app-usage-tracker-query) 中提供了可供 AI 工具使用的 skill 文件，方便通过 CLI 访问本地使用数据。
